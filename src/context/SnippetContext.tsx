...
interface SnippetContextType {
  categories: Category[];
  snippets: Snippet[];
  addCategory: (category: Omit<Category, 'id' | 'createdAt' | 'isDefault' | 'authorId' | 'authorName'| 'authorAvatar'>) => void; // Omit authorId and authorName>) => void;
...
const defaultCategories: Category[] = [
  {
    ...
    authorId: 'system',
    authorName: 'System', 
    authorAvatar: undefined, // No avatar for system
  },
  {
    id: 'omnia',
    name: 'Omnia',
...
export const SnippetProvider = ({ children }: { children: ReactNode }) => {
  // UPDATED KEY TO V5 TO RESET BROKEN DATA
  const [categories, setCategories] = useLocalStorage<Category[]>('snippet-categories-v5', defaultCategories);
  const [snippets, setSnippets] = useLocalStorage<Snippet[]>('snippet-data-v5', seedSnippets);

  const addCategory = useCallback((category: Omit<Category, 'id' | 'createdAt' | 'isDefault' | 'authorId' | 'authorName' | 'authorAvatar'>) => {
    const newCategory: Category = {
      ...
      authorAvatar: currentUser.avatar, // Save correct avatar
    };
  ...
  const addSnippet = useCallback((snippet: Omit<Snippet, 'id' | 'createdAt' | 'updatedAt' | 'reviews'>) => {
    const newSnippet: Snippet = { ...snippet, id: `snip-${Date.now()}`, reviews: [], createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
    setSnippets(prev => [...prev, newSnippet]);
  }, [setSnippets]);

  const updateSnippet = useCallback((id: string, updates: Partial<Snippet>) => {
    setSnippets(prev => prev.map(s => s.id === id ? { ...s, ...updates, updatedAt: new Date().toISOString() } : s));
  }, [setSnippets]);
  ...
  const addReview = useCallback((snippetId: string, review: Omit<Review, 'id' | 'createdAt'>) => {
    ...
    setSnippets(prev => prev.map(s => s.id === snippetId ? { ...s, reviews: [...s.reviews, newReview], updatedAt: new Date().toISOString() } : s));
  }, [setSnippets]);
  ...
  return (
    <SnippetContext.Provider value={{
      categories, snippets, addCategory, deleteCategory, addSnippet, updateSnippet, deleteSnippet, addReview, getSnippetsByCategory, getSnippetById, getCategoryById,
    }}>
      {children}
