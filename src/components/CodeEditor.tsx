...
export const CodeEditor = ({ code, language, isAuthor, onSave }: CodeEditorProps) => {
  ...
  return (
    // FIX 1: Changed to flex-col and h-full to force the box to fill parent height
    <div className="code-container relative h-full flex flex-col bg-[#282c34] border border-border rounded-lg overflow-hidden">
      ...
      {/* Code Area - flex-1 makes it fill the remaining space */}
      <div className="flex-1 overflow-hidden">
        <CodeMirror
          ...
          editable={isEditing}
          theme={theme === 'dark' ? 'dark' : 'light'}
          height="100%" // FIX 2: Force CodeMirror to take full height of the container
          basicSetup={{
            lineNumbers: true,
            highlightActiveLineGutter: true,
            highlightActiveLine: isEditing,
          ...
          className="text-sm h-full" // Ensure css class also dictates full height
        />
      </div>
    </div>
  );
